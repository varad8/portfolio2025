  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-6 py-4 md:px-12">
        ...
        {/* Desktop Links */}
        <nav className="hidden md:flex space-x-6 text-slate-dark">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
        ...
        {/* Profile Menu (right side, desktop only) */}
        <div className="block ml-30 md:ml-4">
        ...
        {!user && (
          <Link
            href="/user/login"
            className="block bg-orange-accent text-white py-2 px-3 rounded-full hover:bg-orange-accent/80 transition-colors"
          >
            Log In
          </Link>
        )}
